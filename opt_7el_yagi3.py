...
def apply_ground(nec, ground_variant):
    ...
    if ground_variant == "real2":
        # GN card: type=2 (Sommerfeld/Norton), radials=0, F1=rel. dielectric,
        # F2=conductivity (S/m). epsr/sigma MUST go in the F1/F2 slots.
...
# ---------------------------------------------------------------------------
# Radiation-pattern / gain reading.
#
# The installed necpp build exposes:
#     nec_gain(nec, freq_idx, theta_idx, phi_idx)  -> total power gain in dBi
#     nec_gain_max(nec, freq_idx), nec_gain_mean(...), ...
# A radiation pattern must first be requested with an RP card.  We run ONE full
# upper-hemisphere RP sweep per geometry and read every grid point back - this
# is both correct and far faster than a separate solve per (elev, azimuth).
# ---------------------------------------------------------------------------

# Upper-hemisphere sampling grid.
_RP_THETA0_DEG = 0.0      # theta measured down from the zenith (+Z)
_RP_DTHETA_DEG = 5.0
_RP_NTHETA = 19           # 0..90 deg -> horizon at the last row
_RP_PHI0_DEG = 0.0
_RP_DPHI_DEG = 5.0
_RP_NPHI = 72             # 0..355 deg

# The Yagi boom lies along +Y, so the main beam fires toward phi = 90 deg.
_FWD_PHI_DEG = 90.0
_REAR_PHI_DEG = 270.0
_FB_SECTOR_HALFWIDTH_DEG = 30.0   # +/- window around forward / rear azimuth


def _pattern_ground_variants(use_real_ground):
    # Real-ground patterns are what we want; the trailing fallbacks only matter
    # if a particular necpp build refuses a finite pattern for that ground model.
    if use_real_ground:
        return ["real2", "real0", "perfect", "free"]
    return ["free"]


def _phi_indices_in_sector(center_phi_deg, halfwidth_deg):
    cols = []
    for ip in range(_RP_NPHI):
        phi = (_RP_PHI0_DEG + ip * _RP_DPHI_DEG) % 360.0
        delta = abs((phi - (center_phi_deg % 360.0) + 180.0) % 360.0 - 180.0)
        if delta <= halfwidth_deg + 1e-9:
            cols.append(ip)
    return cols


def _gain_grid_from_context(nec):
    """Run one full upper-hemisphere RP sweep and return an (NTHETA, NPHI)
    numpy array of total power gain in dBi (NaN where unavailable)."""
    ret = necpp.nec_rp_card(
        nec,
        0,                  # normal calculation mode (ground per GN card)
        _RP_NTHETA,
        _RP_NPHI,
        0, 0, 0, 0,         # XNDA: total gain, no normalization, power gain, no avg
        _RP_THETA0_DEG,
        _RP_PHI0_DEG,
        _RP_DTHETA_DEG,
        _RP_DPHI_DEG,
        0.0,                # radial distance (0 -> far field)
        0.0,                # gain normalization factor
    )
    if ret != 0:
        return None

    grid = np.full((_RP_NTHETA, _RP_NPHI), np.nan, dtype=float)
    for it in range(_RP_NTHETA):
        for ip in range(_RP_NPHI):
            try:
                grid[it, ip] = float(necpp.nec_gain(nec, 0, it, ip))
            except Exception:
                grid[it, ip] = np.nan
    return grid


def _pattern_cache_key(lengths_ft, spacings_ft, height_ft, freq_mhz, use_real_ground):
    return (
        _round_tuple(lengths_ft),
        _round_tuple(spacings_ft),
        round(float(height_ft), 6),
...
def _solve_pattern_metrics_once(lengths_ft, spacings_ft, height_ft, freq_mhz, ground_variant):
    nec = _build_nec_context(lengths_ft, spacings_ft, height_ft, freq_mhz, ground_variant)
    try:
        grid = _gain_grid_from_context(nec)
    finally:
        necpp.nec_delete(nec)
    ...
    fwd_cols = _phi_indices_in_sector(_FWD_PHI_DEG, _FB_SECTOR_HALFWIDTH_DEG)
    rear_cols = _phi_indices_in_sector(_REAR_PHI_DEG, _FB_SECTOR_HALFWIDTH_DEG)

    fwd_vals = grid[:, fwd_cols]
    rear_vals = grid[:, rear_cols]
    fwd_vals = fwd_vals[np.isfinite(fwd_vals)]
    rear_vals = rear_vals[np.isfinite(rear_vals)]
    if fwd_vals.size == 0 or rear_vals.size == 0:
        return None

    forward = float(np.max(fwd_vals))
    rear = float(np.max(rear_vals))
    return forward, rear, forward - rear


def estimate_pattern_metrics(lengths_ft, spacings_ft, height_ft, freq_mhz, use_real_ground,
                             elev_samples_deg=None):
    """Return (forward_gain_dbi, rear_gain_dbi, front_to_back_db) from a single
    full upper-hemisphere NEC radiation-pattern solve.

    Uses the requested ground model; if the installed necpp build cannot
    produce a finite pattern for it, falls back real0 -> perfect -> free so the
    optimizer always has gain / front-to-back to work with.  `elev_samples_deg`
    is accepted for backwards compatibility but is no longer used.
    """
    key = _pattern_cache_key(lengths_ft, spacings_ft, height_ft, freq_mhz, use_real_ground)
    if key in NEC_GAIN_CACHE:
        return NEC_GAIN_CACHE[key]

    for ground_variant in _pattern_ground_variants(use_real_ground):
        try:
            res = _solve_pattern_metrics_once(
                lengths_ft, spacings_ft, height_ft, freq_mhz, ground_variant
            )
        except Exception:
            res = None
        if res is not None and all(np.isfinite(v) for v in res):
            NEC_GAIN_CACHE[key] = res
            return res

    fallback = (np.nan, np.nan, np.nan)
    NEC_GAIN_CACHE[key] = fallback
    return fallback
