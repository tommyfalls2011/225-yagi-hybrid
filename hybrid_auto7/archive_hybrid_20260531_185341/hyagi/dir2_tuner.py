...
def score_candidate(summary, pattern):
    """
    DIR2 score. Higher is better.
    """
    score = 0.0

    # SWR coverage is mandatory
    score += summary.points_under_2p0 * 120.0
    score += summary.points_under_1p5 * 80.0

    # Penalize high SWR
    score -= summary.max_swr * 80.0
    score -= summary.avg_swr * 30.0

    # Reward gain
    score += pattern.real_gain_dbi * 160.0

    # Reward front/back, capped
    score += min(pattern.front_back_db, 40.0) * 15.0

    # Keep max lobe forward
    phi_error = abs(pattern.max_gain_phi_deg - 90.0)
    score -= phi_error * 20.0
...
def run_one(base_row, base_elements, dir2_pos, dir2_len, stage, engine, ant, freqs):
...
def tune_dir2(base_run_id, level="quick"):
    ...
    if level == "deep":
        pos_values = frange(190, 300, 2)
        len_values = frange(165, 200, 1)
    elif level == "normal":
        pos_values = frange(200, 290, 2.5)
        len_values = frange(170, 198, 1.5)
    else:
        pos_values = frange(205, 285, 5)
        len_values = frange(172, 196, 2)
    ...
    for pos, length in product(pos_values, len_values):
        ...
        try:
            ...
            failed += 1
            print(f"FAILED {count}/{total} DIR2pos={pos} DIR2len={length}: {exc}")

    if best_info is None:
        print("No DIR2 candidate succeeded.")
        return None

    row, summary, pat, score, pos, length = best_info

    print()
    print("Best DIR2 candidate")
    print("===================")
    print(f"Run id:              {row['id']}")
    print(f"Score:               {score:.1f}")
    print(f"DIR2 position:       {pos:.3f} in")
    print(f"DIR2 length:         {length:.3f} in")
    ...
    return {
        "run_id": row["id"],
        "score": score,
        "dir2_position_in": pos,
        "dir2_length_in": length,
        "summary": summary,
        "pattern": pat,
        "failed": failed,
    }
