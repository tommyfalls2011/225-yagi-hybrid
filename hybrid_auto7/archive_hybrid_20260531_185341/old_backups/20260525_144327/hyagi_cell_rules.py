...
# === XFRMR_COUPLER_BOUNDS_V1 ===
def enforce_xfrmr_coupler_bounds(elements, rules):
    """Reject any geometry where XFRMR or COUPLER spacing is outside
    operator-defined real-world bounds (independent of global MAX_SPACING_IN).

    elements: iterable with attrs .name (str) and .position_in (float, inches from DE).
    rules:    dict from physics_rules.json
    Returns (ok: bool, reason: str).
    """
    x_min = float(rules.get("XFRMR_MIN_SPACING_IN", 4.0))
    x_max = float(rules.get("XFRMR_MAX_SPACING_IN", 32.0))
    c_min = float(rules.get("COUPLER_MIN_SPACING_IN", 4.0))
    c_max = float(rules.get("COUPLER_MAX_SPACING_IN", 32.0))
    for e in elements:
        nm = (getattr(e, "name", "") or "").upper()
        # spacing = absolute distance from DE (DE itself at 0)
        sp = abs(float(getattr(e, "position_in", 0.0)))
        if "XFRMR" in nm and not (x_min <= sp <= x_max):
            return False, f"XFRMR spacing {sp:.2f}\" outside [{x_min}, {x_max}]"
        if "COUPLER" in nm and not (c_min <= sp <= c_max):
            return False, f"COUPLER spacing {sp:.2f}\" outside [{c_min}, {c_max}]"
    return True, "ok"
