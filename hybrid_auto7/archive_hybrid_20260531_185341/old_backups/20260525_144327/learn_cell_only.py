...
def make_cell():
    ...
    csp = 10.0
    return [
        Element("XFRMR", de_pos - xsp, DE_BASE_IN - 6.0),
...
def run_cell_tune():
    ...
    log()
    log("Stage 1: XFRMR spacing, DE fixed at DE_START")
    ...
    log()
    log("Stage 2: COUPLER spacing, DE fixed at DE_START")
...
def save_outputs(best_elements, best_eval, all_logs):
    ...
    nec_path = MODELS_DIR / f"learn_cell_best_{tag}.nec"
    json_path = OUT_DIR / f"learn_cell_best_{tag}.json"
    log_path = OUT_DIR / f"learn_cell_moves_{tag}.jsonl"
    summary_path = OUT_DIR / f"learn_cell_summary_{tag}.txt"
    ...
    # Ground-reflected pattern: expensive, run once for best design only
    real_gain_dbi = None
    peak_elev_deg = None
    try:
        pat = evaluate_pattern_for_cell(best_elements, CENTER_FREQ_MHZ, ant)
        ...
        if real_gain_dbi is not None and peak_elev_deg is not None:
            log(f"Ground pattern: real_gain={real_gain_dbi:.2f} dBi, "
                f"peak_elev={peak_elev_deg:.1f} deg")
    except Exception as _pat_err:
        log(f"[warn] pattern extraction failed: {_pat_err}")
    ...
    lines = []
    lines.append("CELL PLACEMENT TUNE REPORT")
    lines.append("==========================")
    lines.append("")
...
def main():
    ...
    log("Tuning order: xsp -> csp -> xsp fine -> csp fine -> xlen -> clen -> de_len -> de_only_move -> nudges")
